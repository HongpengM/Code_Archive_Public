...
//    this.baseDOM.appendChild(this.gui.domElement)
  ...
  async addTestSphere (x, y, z) {
    ...
    let resp = await axios.get('http://localhost:8080/asset/neo3d.txt')
    let data = resp.data.split('\n').map(i=>{
      return i.split(',').map(it=>{return parseFloat(it)})
    })
    console.log(data)
    const pointSphere = (() => {
      let geometry = new THREE.SphereBufferGeometry(2)
    ...
    let clones = []
    data.forEach(d=>{
      let meshClone = pointSphere.clone()
      meshClone.position.set(d[0]*200, d[1]*200, d[2]*200)
      clones.push(meshClone)
    })
